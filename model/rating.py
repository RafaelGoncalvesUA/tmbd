...
def predict_rating_for_user(rating_regressor, categories, user_preferences_mean):
    cat_features = {cat: 1 if cat in categories else 0 for cat in all_categories}
    
    user = {
        col + '_user_pref'
        if col in all_categories else col: val
        for col, val in user_preferences_mean.items()
    } 
